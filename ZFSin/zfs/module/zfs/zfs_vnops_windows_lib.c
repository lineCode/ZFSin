...
* If only_driveletter or only_volume_name is set TRUE, every mountpoint will be 
* checked with MOUNTMGR_IS_DRIVE_LETTER or MOUNTMGR_IS_VOLUME_NAME and discarded if not valid
* only_driveletter and only_volume_name are mutual exclusive
*/
NTSTATUS mountmgr_get_mountpoint(PDEVICE_OBJECT mountmgr, PUNICODE_STRING devpath, char *savename, BOOLEAN only_driveletter, BOOLEAN only_volume_name)
{
...
*/
NTSTATUS mountmgr_get_volume_name_mountpoint(PDEVICE_OBJECT mountmgr, PUNICODE_STRING devpath, char *savename)
{
...
/*
* go through all mointpoints (IOCTL_MOUNTMGR_QUERY_POINTS) and check if our driveletter is in the list
* return 1 if yes, otherwise 0
*/
NTSTATUS mountmgr_is_driveletter_assigned(PDEVICE_OBJECT mountmgr, wchar_t driveletter, BOOLEAN *ret) {
	MOUNTMGR_MOUNT_POINT point = { 0 };
	MOUNTMGR_MOUNT_POINTS points;
	PMOUNTMGR_MOUNT_POINTS ppoints = NULL;
	int len;
	*ret = 0;
	...
	if (Status == STATUS_SUCCESS) {
		...
		for (int Index = 0; Index < ppoints->NumberOfMountPoints; Index++) {
			...
			ULONG len = 0;
			RtlUnicodeToUTF8N(mpt_name, MAXPATHLEN, &len, SymbolicLinkName, ipoint->SymbolicLinkNameLength);
			mpt_name[len] = 0;
			char c_driveletter;
...
*/
NTSTATUS mountmgr_assign_driveletter(PUNICODE_STRING device_name, wchar_t driveletter) {
...
*/
NTSTATUS SetNextDriveletterManually(PDEVICE_OBJECT mountmgr, PUNICODE_STRING device_name) {
	NTSTATUS status;
...
int zfs_vnop_mount(PDEVICE_OBJECT DiskDevice, PIRP Irp, PIO_STACK_LOCATION IrpSp)
{
   ...
   //RtlInitUnicodeString(&mountp, L"\\DosDevices\\F:");
	...
	// Set the mountpoint if necessarry
#if 0
...
int zfs_windows_unmount(zfs_cmd_t *zc)
{
	...
	if (getzfsvfs(zc->zc_name, &zfsvfs) == 0) {
		...
		// Query MntMgr for points, just informative
		RtlInitUnicodeString(&name, MOUNTMGR_DEVICE_NAME);
		NTSTATUS status = IoGetDeviceObjectPointer(&name, FILE_READ_ATTRIBUTES, &fileObject,
			&mountmgr);
		char namex[200] = "";
